...
// 1. Define a whitelist of columns that are allowed to be sorted. This is a SECURITY measure.
$allowed_sort_columns = ['id', 'name', 'mobile', 'course', 'dob', 'submission_date'];

// 2. Determine the sort column. Default to 'submission_date' if not set or invalid.
$sort_col = 'submission_date'; // Default sort column
if (isset($_GET['sort']) && in_array($_GET['sort'], $allowed_sort_columns)) {
    $sort_col = $_GET['sort'];
}

// 3. Determine the sort order. Default to 'DESC'. Toggle if the same column is clicked again.
...
$limit = 10;
$count_stmt = $pdo->query("SELECT count(*) FROM enrollments");
...
// The SQL now uses the validated $sort_col and $sort_order variables.
// NOTE: Column names in ORDER BY cannot be bound with parameters, which is why the whitelist is essential for security.
$sql = "SELECT * FROM enrollments ORDER BY {$sort_col} {$sort_order} LIMIT :limit OFFSET :offset";
...
<div class="card">
    <div class="card-header">
        ...
        <div class="card-tools">
            <a href="create.php" class="btn btn-primary">Add New Enrollment</a>
        </div>
    </div>
        ...
        <table class="table table-bordered table-striped">
            <thead>
                <tr>
                    ...
                    <?php sort_link('Mobile', 'mobile', $sort_col, $sort_order); ?>
                    <?php sort_link('Course', 'course', $sort_col, $sort_order); ?>
                    <?php sort_link('DOB', 'dob', $sort_col, $sort_order); ?>
                    <th>Actions</th>
                </tr>
            </thead>
            <tbody>
                <?php if ($enrollments): ?>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($enrollment['mobile']) ?></td>
                        <td><?= htmlspecialchars($enrollment['course']) ?></td>
                        <td><?= htmlspecialchars($enrollment['dob']) ?></td>
                        <td>
                            <a href="edit.php?id=<?= $enrollment['id'] ?>" class="btn btn-sm btn-warning">Edit</a>
                            <a href="delete.php?id=<?= $enrollment['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
